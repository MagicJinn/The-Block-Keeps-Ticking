...
import magicjinn.blockkeepsticking.api.CampfireAccess;
import magicjinn.blockkeepsticking.util.TimeUtils;
...
public class ChunkLifeSimulator {
		...
		for (Map.Entry<BlockPos, BlockEntity> entry : chunk.getBlockEntities().entrySet()) {
			...
			if (blockEntity instanceof CampfireBlockEntity && 
				...
				state.get(net.minecraft.block.CampfireBlock.LIT) && 
					BlockKeepsTicking.CONFIG.isCampfiresEnabled()) {
	...
	private static void simulateEntities(WorldChunk chunk, long timePassed) {
		...
		for (Entity entity : entities) {
			if (BlockKeepsTicking.CONFIG.isMobGrowingEnabled() && entity instanceof PassiveEntity) {
			...
			if (BlockKeepsTicking.CONFIG.isChickenEggLayingEnabled() && entity instanceof ChickenEntity) {
	...
	private static void simulateCropGrowth(ServerWorld world, WorldChunk chunk, long timePassed) {
		if (!BlockKeepsTicking.CONFIG.isCropsGrowingEnabled())
			return;
			...
			for (int x = 0; x < 16; x++) {
				for (int y = 0; y < 16; y++) {
					for (int z = 0; z < 16; z++) {
						...
						if (!canRandomTick(state)) continue;
